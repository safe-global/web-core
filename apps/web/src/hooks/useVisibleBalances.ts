...
export const useVisibleBalances = (): {
  ...
  return useMemo(
    () => ({
      loading,
      balances: {
        items: filterHiddenTokens(balances.items, hiddenTokens),
        fiatTotal: balances.fiatTotal ? getVisibleFiatTotal(balances, hiddenTokens) : '',
      },
    }),
    [balances, loading, hiddenTokens],
  )
}
